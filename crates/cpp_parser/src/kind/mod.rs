...
impl CppKind {
    ...
    fn syntax_from_raw(raw: u16) -> Option<CppSyntaxKind> {
        // `RegisterSpec` is the last variant, so a discriminant is valid exactly when it is <= it. (Every kind
        // that is added goes **after** the one named here, never before it: the variants before it are the ones a
        // stored discriminant could already mean.)
        if raw > CppSyntaxKind::RegisterSpec as u16 {
            return None;
        }
...
mod tests {
    ...
    fn out_of_range_raw_is_rejected() {
        ...
        assert!(
            CppKind::try_from_raw(CppSyntaxKind::RegisterSpec as u16 + 1 + SYNTAX_KIND_TAG).is_none()
        );
