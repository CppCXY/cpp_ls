...
### B41（待办）：宏调用**省略分号**，而宏体自带语句

```cpp
NUMBER_OPTION(indent_size)      // 宏展开成一条完整语句（自带 `;`），于是调用处不写分号
NUMBER_OPTION(tab_width)
```

**现象**：`LuaStyle.cpp` 剩下的 **64 条**全是它：`NAME(args)` 后面紧跟下一条语句（或 `}`），报 `expected ; after expression`，随后级联。

**成因**：严格说这是**不合法的 C++**——调用表达式后面必须有 `;`——但宏体自带分号时，预处理后的代码是合法的，而 parser 不做预处理。所以这不是"缺规则"，而是"要不要为宏再开一次口子"。

**性质**：**取舍**，不是缺口，所以登记成待办而不是直接修。两个方向各有代价：

* 收下它：所有**漏写 `;` 的调用语句**都会被静默吞掉——那是最常见的手误之一，诊断会明显变差；
* 不收：宏密集的文件（这一份 64 条）继续报错，但每一处都是真实存在的"这里依赖宏体的分号"。

**如果要修，建议的方向**（和 B36 同一条约定，只是更危险）：只对**宏书写的名字**（全大写下划线）放行，且后面必须紧跟一个不能继续表达式的 token；即便如此也会吞掉 `CHECK(x)` 这类漏分号。**建议先问**：这个工程里宏体自带分号的写法有多普遍。
...
| — | 宏调用省略分号（宏体自带语句）——**取舍**，两个方向各有代价，见 B41 | B41 | 待定 | 待办（需决定） |
