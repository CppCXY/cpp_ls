...
### P1 让闭包干净解析（**进行中**：48 → 113 个文件干净；第十七/十八轮之后 335 → **432**，128 个文件那份 80 → **113**）
...
> **逐批的干净/报错/消息三个数在 [`roadmap.md`](roadmap.md) §2.0**，那一份是**唯一**的账（同一批数字只写一处，
> 免得两处对不上）。这里只留"哪些文件变干净了"——那是标准库这一条线自己的收获。
...
error_constants.h（枚举表里的指令）  随 B77 变干净（1 个）；初始化式里的指令**量过三版后撤回**（见 B77）
ranges_base.h、concepts（复合要求里的花括号）  随 B78 变干净（2 个）
type_traits 的错 13 → 9 条（随 B79；首错 198 → 986 行——这一类是把首错往后推）
node_handle.h（随 B80；`::new (…) T(a.c())` 那一形）——两份清单上都变干净
```
...
**B78 是一条判据用错了位置**：约束里的 `{` 被当成"被约束的那个定义的身体"（这条判据本身没错，
`requires C<T> { }` 若读成 `C<T>{}` 函数体就丢了），但**复合要求自己的 `{ }` 里面**那个 `{` 不可能是身体——
它已经嵌在要求的括号里一层了。例外用现成的证据写成：`Requirement` 节点开着的时候就是那种情形。
`ranges_base.h` 一处首错带的 24 条诊断随它消失，两份清单的消息各 −25。

**B79 补的两处都是"规则只覆盖了一半"**：基类子句只读名字，而 `decltype(…)` 是 base-specifier 的另一半（libstdc++
的 `__or_`/`__and_` 全用它）；模板实参表的指令接缝只在参数**之前**，而 `#if` 也能站在参数之后、逗号之前——
与 B77 的枚举表同形。首错因此往后推：`type_traits` 13 → 9 条错，首错 198 → 986 行。
...
**下一步的队列在 [`roadmap.md`](roadmap.md) §2.0 的"下一批"**（`std_probe` 随时重排，且会打印文件名）；这一份只保留"为什么这么做"与每次普查的数字。
