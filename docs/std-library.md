...
* **但这里有一条定理，而且它不需要宏环境**：*`#if` 的每个分支都写了同一个名字、同一种 kind ⇒ 这个条件块之后
  这个名字的宏状态与分支无关*。`#ifndef NAME / #define NAME` 是它的特例：条件成立则这里定义，不成立则它
  已经被别的头文件定义过了——**两种情况之后它都是宏**。所以正确的做法不是猜，而是在事实层记下这个结论
  （`MacroFact` 加一个布尔字段，抬 `CODEC_VERSION`），由引用查询使用（定义查询仍然要知道"哪一条在生效"，
  那依然是有条件的）。这一步做完，上面四个宏的 8 000 多条"可能"会变成"使用"。
