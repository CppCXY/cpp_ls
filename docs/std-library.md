...
### P1 让闭包干净解析（**已开始**：48 → 78 个文件干净）

**按形状放宽，而不是接表**——这是这一轮最重要的一条计划修正，理由见下。

两条规则都落在 parser 里，都**不依赖任何宏表**，因为标准库的宏名字定义在 `c++config.h`（一个**被包含**
的文件）里，本文件的 `MacroNames` 没见过它，而外部表（`symbols.rs`）也没接到 includes 上。能知道的只有
形状，而形状在下面这两处是决定性的：

| 形状 | 规则 | 代价 |
|---|---|---|
| `namespace std _GLIBCXX_VISIBILITY(default) {`（**每个** libstdc++ 头的第一行） | 名字与 `{` 之间允许宏形状的 token（名字，或名字 + 一个括号组），而且**只有落在 `{` 上才算** | 合法 C++ 在名字与 `{` 之间什么都没有，所以两种读法是"`namespace std {`"与"报错"，接受宏不花任何代价 |
| `_GLIBCXX_BEGIN_NAMESPACE_VERSION` 独占一行（在一处或另一处分支里展开成 `namespace __8 {`，在另一处**什么都不是**） | 名字后面跟的东西**能开始一个声明**（锚点或类型关键字）或**结束作用域**（`}`/`#`/EOF）时，读成 `MacroCall` | 一律**不在函数体内**生效：那里 `COUNT` 后面跟 `return` 是漏了 `;`，读成宏就会把一个人该修的错吞掉 |

**实测**（`examples/std_probe.rs`，同一份 185 个文件的闭包）：

```text
之前：干净 48 / 报错 134     第一个错里"行内有闭包定义的宏"占 58%
之后：干净 78 / 报错 104     同样是 185 个文件 / 111,420 行
```

`bits/stl_algobase.h` 一个文件从 190 条错降到零头——那 190 条全部来自第 83 行的命名空间头。

**这条规则被改写了一次，两次的错都钉在 `gaps.rs` 里**，因为两次都是"看起来对、其实把另一种读法吃了"：

* 第一版问的是"名字后面还有没有 declarator"。`x = 1;` 就是它的代价：`=` 后面没有 declarator，于是赋值被
  读成宏，`=` 左边就空了。正确的问题是"后面跟的东西能不能**开始**一个新构造"——`=`、`*`、`(`、`.`、`[`
  都是**延续**，所以它们不在表里；
* 第二版把 `FOO(x);` 和 `TEST(A, B) { … }` 也吃掉了：前者是最令人头疼的解析（本项目读成声明），后者是
  gtest 那种"块就是体的定义"。两处都由别的规则拥有，而且都有既有测试——所以括号组后面是什么，现在由
  `kind_after_the_group` 一次走查回答，四个调用方共用。

**下一步的队列**（按第一个错分类，`std_probe` 随时重排）：`__attribute__((…))`（家族二，`c++config.h`
自己的第一个错就是它）、声明符之后的尾随宏（`… ) const _GLIBCXX_NOEXCEPT`）、requires 表达式里的运算符
函数调用（`{ operator<=>(…) }`）、`x.~T()` 与 `x.operator->()`、变量模板的偏特化。

**判据**：`std_probe` 的 `failing` 往 0 走。**0 是目标，不是 90%**——理由见下面"不变量 3 的粒度"。
