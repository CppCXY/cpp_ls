...
2. **停下来，去处理"不变量 3 的粒度"**：88/185 已经过半。既然闭包短时间内到不了 0，那"只相信干净解析的文件"这条不变量对标准库就仍然不成立——**这才是真正挡着语义查询的那道门**（见下）。处理它（比如给事实记一个解析干净度、由查询层降权）比再修十个构造更早解锁能力；
3. **两件事交替做**：一轮 parser、一轮语义，避免长时间只在一条线上。

倾向 **2 或 3**：P1 现在的边际收益已经降到"每轮 1–3 个文件"，而"标准库的声明可不可信"是下一层的**前置条件**。这个判断留在这里，由下一轮决定。
