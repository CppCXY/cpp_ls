...
Rust 侧 `cargo test --workspace` = **992 个测试 / 34 个套件全绿**。

**门禁三条 + 一条**（改完必须全绿，`index-design.md` §门禁有同样的表）：

```bash
cargo test --workspace                     # 992 个测试，34 个套件
...
1. **新登记的 B42/B43**（这一轮修恢复时量出来的两个真缺口，各带最小复现）：
   `void f() try { } catch (...) { }`（函数定义里的 `try`——`try` 有语句规则，但声明符与函数体之间那个位置没接）
   与 `if (int x = g())`（条件里的声明）。两个都是"接缝/二选一"型的缺规则，各值 1–3 个文件。
2. 各文件的**下一条**：`bits/move.h:233`（函数体里那个没有 `;` 的宏——现在只是**一条诊断**，块与类都不再丢）、
...
3. 队列里**还没碰**的：§2.2（GNU 类型拼写：`__typeof__` / `__int128`）、§2.5（模板参数表里的宏）。
4. 剩下 29 个"超过五个错"的文件——那些是级联，按第 11 条先归类再动手。
...
1. **宏这条线继续走到底**（用户的原始指示："真正的宏"）：
   * ~~条件块的分支结论~~（**已做完**，§3.5b）：333 个宏名"定得住"，2 626 条引用从"可能"变成"使用"。
     这一步不需要宏环境，量出来的收益比预期的 P3 便宜得多。
   * **`winnt.h` 那一族解析错**（§3.5b 末尾量出来的下一步）：定义 `STDMETHODCALLTYPE`/`WINAPI`/`__attribute__`
     这些最常用宏的文件有几百条语法错，而错的代价是**指令丢失**、嵌套不可信、规则一条都不认。这是 parser
     的长尾（§2.1 第 1 条那一族），修一个文件就解锁一批宏。
   * **宏展开进事实**（`BindingOrigin::MacroExpansion` 词汇已就位）——P4，最贵；上面那条定理让 P3（宏环境进键）
     变得不那么急。
