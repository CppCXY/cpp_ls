...
`干净 100 / 报错 28`，消息总数 **283**；每文件错误数 `干净 100 | 只有一个 5 | 两到五个 14 | 超过五个 9`；
`%TEMP%\cppls-indexed.txt` 的 455 个文件（分析闭包）——`干净 400 / 报错 55`，消息总数 **352**。
Rust 侧 `cargo test --workspace` = **1048 个测试 / 34 个套件全绿**。
...
`::new (…) T(args)` 那种"分配式的类型是括号表达式"、`0.0f16` 那种 `_Float16` 字面量后缀、
`{ .__v = __A }` 那种**指定初始化式**、`_MM_REDUCE_OPERATOR_BASIC_EPI16 (+)` 那种**把运算符当宏实参**的调用、
`noexcept(…)` 跨行且不在接缝上的那一族、B65（声明的尾巴按分支写），以及 `register` 说明符。
**最大的一族**（6 个文件）是 `_GLIBCXX11_DEPRECATED_SUGGEST(...)` / `__glibcxx_function_requires(...)` 这种
"名字来自被包含的头文件、却当成语句/声明片段用"的宏——它需要**按位置**的宏证据，而 parser 现在只有
"本文件的 `#define`"和"调用方的表"两种；产品路径目前**不给 parser 供表**（`std_probe` 与 `FileIndexer`
都用 `ParserConfig::default()`），所以这一族要么等索引把闭包的宏按位置接进来，要么就得靠形状放宽——
而形状放宽会把"漏写分号"读成宏，那正是 B41 记下来的取舍，不该在没有证据时做。
