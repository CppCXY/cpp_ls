...
`干净 122 / 报错 6`，消息总数 **10**（带 seeds）；每文件错误数 `干净 122 | 只有一个 5 | 两到五个 1 | 超过五个 0`；
`%TEMP%\cppls-indexed.txt` 的 455 个文件（分析闭包）——`干净 450 / 报错 5`，消息总数 **20**。
Rust 侧 `cargo test --workspace` = **1086 个测试 / 34 个套件全绿**。

**两份读数**：上面这两份**不带 include 证据**；带证据（`--seeds --closure`，也就是**带索引的产品形态**）的那一份，
455 个文件是 `干净 451 / 报错 4 / 消息 8`（B97 的三处修复 + B90 的读法落地 + B98 的空体限定符 + B100 的两条接缝/拼法 +
B101 的括号声明符宏 + B102 的函数式属性宏 + B103 的枚举量宏 + B104 的三处模板实参读法 + B105 的条件声明收口 +
B106 的 `*` 与名字之间的属性 + B107–B113 的七条），128 个文件那份是 `干净 122 / 报错 6 / 消息 10`。
**几条规则（B91 起）要吃宏的体，而体只有带索引的那条路才有**——比较读数时必须说清是哪一份，别把"没带索引"当成"没效果"。

**门禁三条 + 一条**（改完必须全绿，`index-design.md` §门禁有同样的表）：

```bash
cargo test --workspace                     # 1086 个测试，34 个套件
...
> 表里 B98…B113 的十六行都是**带 seeds** 的读数，第 5、6 列（455 那份）每一行都有单独运行；
> **128 那份只有 B102、B112、B113 单独量过**，中间几行没量，不编数字进去。
> **B104、B105、B107、B108、B109、B110、B111、B113 的行里干净数可能与前一行相同**——那是"读法对了、首错前移了、
> 但文件还没修好"，见下面的叙述；这就是"读数没动"与"什么都没做"的区别。
> **B99 不在表里**：复现修好了、断言也有，但语料一条没动，按"读数变了才算修好"不算数。
