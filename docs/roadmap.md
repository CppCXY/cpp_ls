...
`STDMETHOD(QueryInterface) (THIS_ REFIID riid,LPVOID *ppvObj) PURE;`——**它不是缺陷**：那个读法要 `STDMETHOD` 的**体**，
而体在 `combaseapi.h` 里，只有带索引的那条路才有（B90/B97）。128 个文件那份两份读数都是干净的。
**几条规则（B91 起）要吃宏的体，而体只有带索引的那条路才有**——这就是上面那一个文件的全部原因，
也就是为什么两份读数必须分开报。
...
+ B98  指针限定符位置上的**空体宏**（`*POINTER_32`）      （未单独量）            438    17    63
+ B100 初始化式两侧的指令接缝 + `__attribute` 少一个下划线的拼法 （未单独量）      440    15    61
+ B101 括号声明符里的宏：`(*MACRO NAME)` 与 `(MACRO *NAME)`     （未单独量）      442    13    58
+ B102 说明符位置的**函数式属性宏**（`_GLIBCXX11_DEPRECATED_SUGGEST(…)`）  （未单独量）  443  12  46
+ B103 枚举量名字与 `=` 之间的**宏**（`__GOMP_DEPRECATED_5_1`）          （未单独量）  444  11  41
+ B104 模板实参那一族的**三处读法**（失败的读法不算答案 / `>=` 不是收尾符 / 尖括号只数一处且看前一个 token）
                                                                       118   10    24         444    11    39
+ B105 条件里的声明**必须落在条件的 `)` 上**（`if (NS::fmod(s, T(2)) == 0)`） （未单独量）  444    11    36
+ B106 `*` 与名字之间的**属性**（`void * __attribute__((…)) f (void) { }`）  （未单独量）  445    10    35
+ B107 初始化式/表达式**按分支写**（`#else` 的另一个值、算符位置的指令、括号里的 `#endif`） （未单独量） 447  8  28
+ B108 一条 cast 规则（两处副本，一元那份没有回退）                （未单独量）      448     7    25
+ B109 属性在 declarator-id 与**形参表**之间（`operator== [[nodiscard]] (…)`） （未单独量） 449   6    23
+ B110 说明符宏之后跟**名字**（那个名字后面不是 `(` 就不是声明符）    （未单独量）      449     6    23
+ B111 形参表**之后**的限定符也是"这是函数"的证据                    （未单独量）      450     5    21
+ B112 无参宏 + 块，以及体内**裸的**无参宏语句                   122    6    10         451     4     9
+ B113 数尖括号扫描里的**三目 `:`**                             122    6    10         451     4     8
+ B114 声明/别名/条件的**尾巴按分支写**（形参表、实参表、三目 `:` 支；含终结符共享与"谁拥有分支末尾的 `;`"）
                                                                       128    0     0         455     0     0
+ B115 别名名字与 `=` 之间的**属性宏**                            （未单独量）      455     0     0
+ B116 两处扫描里**配对花括号内的 `;`**                           （未单独量）      455     0     0
+ B117 `the_declaration_has_a_type` 要问 token 级的限定名头        （未单独量）      455     0     0
+ B118 `__extension__` 站在**操作数位置**                        128    0     0         455     0     0
```

> 表里 B98…B118 的二十一行都是**带 seeds** 的读数，第 5、6 列（455 那份）每一行都有单独运行；
> **128 那份只在 B102、B112–B114、B118 上单独量过**，中间几行没量，不编数字进去。
> **有几行的干净数与前一行相同**——那是"读法对了、首错前移了、但文件还没修好"，见下面的叙述；
> 这就是"读数没动"与"什么都没做"的区别。**B99 不在表里**：复现修好了、断言也有，但语料一条没动，
> 按"读数变了才算修好"不算数。
> **最后两行是零错误**：两份带 seeds 的清单现在都是干净的。
...
**B114–B118（收尾的五条，455 那份 451/4 → 455/0、128 那份 122/6 → 128/0，两份都是零错误）**：
主体是 **B114 的"尾巴按分支写"**——一个构造的尾巴（最后一个元素 + 关闭它的 token + 声明末尾的 `;`）在 `#if` 的两支里
各写一遍，而**头是共享的**：形参表（`algorithmfwd.h:700`）、模板实参表（`stl_iterator.h:3090`）、三目的 `:` 支
（`type_traits:2269`）、以及**终结符共享**的那一种（`stl_algo.h:4600`：两支只写到 `)`，`{` 在 `#endif` 之后）。
三件事一起才成立，每一件都是量出来的：① **归属**用 B77 学到的形态——答案是**取用式**的
（`take_the_terminator_came_from_a_branch`，由 `expect_semicolon` 取走），而且**进 `Checkpoint`**，因为投机读法真的
会经过设置它的读取器；② **"谁打开了那个条件"**不能让构造自己数（`type_traits` 的那个 `# if` 是真分支里的**表达式**
吃掉的），计数放在读指令的那一处（`note_a_directive_name` → `open_conditionals`）；③ 两道门里的**形状**门要
"分支各写整条声明"的反例钉住（`#if X void f(int a); #else void g(long a); #endif` 曾读成一个表两个实参）。
最后两处细节也是量出来的：`#if`/`#else` 的 `if`/`else` 是 **C++ 关键字**，按 token 种类判断会把它们全漏掉（要按文本，
并用行尾偏移挡住空指令 `#`）；`a_branch_continues_the_tail` 要同时认"`;` + 分支"和"分支直接跟在尾巴后面"两种拼法。
`B115`–`B118` 各值一个首错（别名上的属性宏、配对花括号里的 `;`、token 级的限定名头、`__extension__` 作操作数），
逐条写在 `grammar-gaps.md` 的 **B114–B118** 那一节。

**这一批最好的证据是两个被拔掉的反例**：`gaps.rs` 里原来有两条 `assert_does_not_read_yet`——一条钉着 B65
（`parallel/algorithmfwd.h:700` 的"第二支是片段"）、一条钉着 `bits/stl_iterator.h:3090`——现在都改成了**正例**，
并且各自带上"一个构造、两支的写法都在里面"的计数。**剩下的一个失败不是缺陷**：不带 seeds 的 455 那份里
`commdlg.h:577` 的 `STDMETHOD(QueryInterface) (…) PURE;` 要有 `STDMETHOD` 的体才读得成，而体在 `combaseapi.h` 里
（B90/B97）——带索引的那条路里它是干净的。

**下一步不是"再修一个文件"了**：这两份清单已经量不出东西，该换清单或换队列——要么拿更大的闭包（整份 libstdc++ +
Windows SDK）重新量一遍，要么把 `index-design.md` 的**索引/语义队列**往前推（本节记的都是 parser 的账）。
`grammar-gaps.md` 的族清单里写着这两条，连同"剩下那个文件为什么不算缺陷"。

**B107–B113（七条，455 那份 445/10 → 451/4，消息 35 → 8）**：这一批的共同点是"**偏好写好了，缺的是一条证据或一次统一**"，
四条成对修好四个文件（stl_algobase.h + riemann_zeta.tcc 由 B107、iterator_concepts.h 由 B108、tuple 由 B109、
stl_function.h 由 B110+B111、safe_iterator.h 由 B112），三条只把首错往前推（B113 把 ranges_util.h 从 438 推到 484）。
逐条的现象/成因/做法/读数都写在 `grammar-gaps.md` 的 **B107–B113** 那一节，这里只记三件对下一轮重要的事：

① **B107 的"回滚条件"是这一批最难的一处**：算符位置读指令时，只有**后面真的跟着算符**才收，否则回滚 —— 因为 `#else`
开的是"包着这个表达式的那层构造"的另一个实例，而那一层（声明的初始化式）有自己的接缝要读它。写成"无条件收"时，
`bits/stl_algobase.h:906` 的第二个值就被表达式吃掉了（同一批 token 在前一轮是干净的，这属于"修一个坏一个"的典型，
靠**逐文件对照**当场看见）。

② **B108 是"同一条规则抄了两遍"的又一处**，而且两份**结论相反**（主规则回退成括号表达式，一元规则的副本报"缺操作数"），
于是同一批 token 谁先看见谁说了算：`(V<T>(x))` 成瓦砾、`(W<T>(x))` 却干净 —— 这正是维护约定第 14 条说的那件事。
删掉一份之后 `iterator_concepts.h` 干净。**这一批里唯一"删代码"的修复**。

③ **B112 让 128 那份清单从 118/10/24 跳到 122/6/10**：`debug/safe_iterator.h` 那一族（`_GLIBCXX20_CONSTEXPR_NON_LITERAL_SCOPE_*`）
在读法落地前，块里的每条语句都进了 `InitListExpr`，一个文件能带出十几条消息。

**剩下 4 个（带 seeds 451/4、消息 8）——三个是同一族**：`algorithmfwd.h:704`（形参表的尾巴按分支写，分支里带 `;`）、
`stl_iterator.h:3094`（别名模板实参表的尾巴按分支写，同样带 `;`）、`type_traits:2274`（三目的 `:` 支每支写一遍，
每支带自己的 `;`），加上 `ranges_util.h:484`（变量模板偏特化的名字里，实参是模板形参自己）。前三个是**一件事**：
"**谁拥有分支末尾那个 `;`**"——B77 记着三种试法各自的代价，而 B100/B103/B105/B107 之后**接缝这一侧已经补齐**，
剩下的正是归属。

**B106（`*` 与名字之间的属性）—— +1 个文件，"压到 10 个"这一步就是它**：`lwpintrin.h:43` 写
`extern __inline void * __attribute__((__gnu_inline__, …)) __slwpcb (void) { … }`，而 `eat_cv_qualifiers`（`*`/`&` 之后
那个位置）读 `const`/`volatile`、实现关键字、已知空体宏，**唯独没读属性**——它自己的文档里写着这是"说明符位置那条规则，
晚一个 token"，属性正是写在那里面的。代价分两半：带函数体时是 ``a declarator takes only one initializer``，
**不带函数体时完全静默**——`void * __attribute__((x)) f;` 读成"声明符名叫 `__attribute__`、初始化式 `(x)`、后面那个
`f` 是一个替声明站位的宏"（B72 那一类 A0 静默错树）。做法是一条属性臂（`at_an_attribute` → `parse_attribute_specifier`，
失败回滚，与 `eat_a_macro_suffix` 同一处理）。**量到**：干净 444 → **445**、报错 11 → **10**，逐文件对照只有
`lwpintrin.h` 消失、无一反向，而且它现在**整份文件零错误**；不带 seeds 也同步 443 → **444**。

**B105（条件里的声明必须落在条件的 `)` 上）—— 计数没动，队列前移**：`riemann_zeta.tcc:173` 写
`if (_GLIBCXX_MATH_NS::fmod(__s,_Tp(2)) == _Tp(0))`，而**条件的声明读法成功、并且是通过它自己两道判据合法地成功的**：
`NS::fmod` 是限定名 ⇒ 是类型，`(s, T(2))` 是函数声明符，形参 `s`/`T` 是 NameExpr ⇒ "解析出了名字" ✓，
`T(2)` 是形参 + 直接初始化式 ⇒ "有初始化式" ✓——两条判据问的都是**声明符自己的事件**，而形参表也产生这些事件。
加第三条拒绝：声明之后必须是条件的 `)`（条件的声明没有自己的 `;`，`)` 就是"结束"的唯一标志），否则退回表达式。
**量到**：干净 444 → 444、报错 11 → 11、**消息 39 → 36**，`riemann_zeta.tcc` 的首错 173 → **182**；不带 seeds
443 → **444**（那个文件在不带 seeds 的读数里被这一条修好了）。断言的正例要求树里**零个 `Initializer`**
（误读产出的正是一个"带直接初始化式的形参"），反例是三种**真的是**声明条件。

**B104（模板实参那一族的**三处读法**：一个文件都没修好，但两处错各前移一格）**：455 带 seeds **444 → 444**、
报错 11 → 11、**消息 41 → 39**。三处：① `parse_template_argument` 的早退原来是
`type_read.is_ok() || !continues_a_type(…)`——**失败**的 type 读法（`(__i` 读到 `>=` 放弃）也被当成"实参读完了"，
而它消费的 token 还在事件流里，于是 `>=` 被 split、`>` 收尾、声明没了头（`tuple:2439` 的 ``expected a declarator name``）；
现在只有**成功的**读法能早退，`continues_a_type` 只剩这一个调用者，随之删掉。② 实参表里的拒绝集原来把
`>=`/`>>=` 也算成收尾符，而 **g++ 只把 `>` 与 `>>` 当收尾**（`C<sizeof(int) >= 4>`、`C<(1) >= 2>` 都接受，
`C<1 > 2>` 才是错）——收窄到两个之后括号里的 `>=` 才读得成运算符；"类型实参后面跟 `>=`"（`C<D<int>= 3>`）仍然是
`split_closing_angle` 的活。③ **三处数尖括号的扫描合成一处**（`angle_depth_delta` 多收一个"前一个 token"参数，
判据是"它能不能结束一个模板名"：标识符 / `>` / `>>`）——组里的 `<` 是小于号，数成开层会让扫描**自己在结尾差一层**，
于是"找不到匹配的 `>`"，那个 `<` 被当成小于号，基类子句/类头 body/实参表在 `type_traits:987` 一行上**同时**错。
**结果**：`tuple` 的首错 2438 → **2532**，`type_traits` 的 987 → **2271**（三目两支之间的指令，这条接缝一起补了）
→ **2274**。两个文件都停在**同一个刻意不读的形状**上：一个构造写在 `#if` 的两支里（B77）。所以这一条按
"读数变了才算修好"的标准**不算修好**，只算"读法正确、三条断言钉住、队列往前挪两格"——与 B99 同一类。
`angle_depth_delta` 的这次合并同时还掉了维护约定里那句债（"三个扫描各写一遍同一条规则"）。

**B103（枚举量名字与 `=` 之间的宏）—— +1 个文件**：`omp.h:74` 写
`omp_proc_bind_master __GOMP_DEPRECATED_5_1` / `= omp_proc_bind_primary,`，那个位置**只读了 `[[…]]` 属性**、没读宏，
于是枚举量在那里就结束了。做法是属性读法旁边一句 `while eat_a_macro_suffix(p) {}`——同一个读者、同一个理由
（枚举量名字之后合法的 token 只有 `[[`、`=`、`,`、指令、`}`，名字一个都不是）。**量到**：干净 443 → **444**、
报错 12 → **11**，逐文件对照只有 `omp.h` 消失、无一反向。断言里那个**反例是同一个位置的 `[[…]]` 拼法**：
它必须继续工作，而且不能被数成宏。

**B102（说明符位置上的函数式属性宏）—— +1 个文件，代价是两次被量回来的收窄**：`backward/binders.h:133` 写
`_GLIBCXX11_DEPRECATED_SUGGEST("std::bind")` 在**类型之前**。它是**函数式**宏，所以 B91 那条"体全是说明符"的规则读不到
（函数式宏的体里有自己的形参），`at_an_attribute` 也不认它（体在 `bits/c++config.h` 里）——说明符序列停在这个名字上。
**三个版本，两次失败正是判据的来源**：① 要求 evidence（`is_function_like`）→ 语料**一条没动**，因为 `binders.h`
自己**一个 `#include` 都没有**（探针原话：`evidence false | positional body None | in-force body None | context 0 seeds`）；
② 只看形状 → `objbase.h:95`、`ole2.h:58` **反向**（`WINOLEAPI_(…)` 那一族被抢走）；③ 要求**两张表都说不出这个体**
（`macro_evidence` **和** `macro_body_kinds_at` 都 None，因为 `WINOLEAPI_` 是**条件分支里的 `#define`**，到使用点是"生效的体"
而不是"定义"）+ **组后面跟的是说明符**（不是名字）→ 干净 442 → **443** / 报错 13 → **12**，只有 `binders.h` 消失、无一反向。
"组后面跟名字"的那一半**明确留给** `a_macro_call_begins_the_declaration`（它读完调用就结束序列）：写成"名字或说明符都收"
时，`WINOLEAPI_(void) CoFreeLibrary (HINSTANCE hInst);` 变成调用表达式 + ``expected `;` after expression``——**四个已有测试
当场抓住**（`gaps.rs`），这也是"读数没动不等于没坏"的一个例子。反例钉住的是更坏的一种：`CHECK(1);` 在函数体里必须
**仍然是调用**，否则它读成"没有声明符的声明"——无声、无损、且用户写的调用在树里根本不存在。

**B101（括号声明符里的宏：`(*MACRO NAME)` 与 `(MACRO *NAME)`）—— +2 个文件**：`exception:87` 写
`typedef void (*_GLIBCXX11_DEPRECATED unexpected_handler) ();`，`combaseapi.h:358` 写
`typedef HRESULT (STDAPICALLTYPE *LPFNGETCLASSOBJECT) (REFCLSID, REFIID, LPVOID *);`，两个宏的体都**在生效**
（`__attribute__((__deprecated__))` / `__stdcall`）。`(MACRO NAME)` 这条路早就有了（`winperf.h` 的 `(WINAPI PM_OPEN_PROC)`），
缺的是宏相对**声明符自己的运算符**的另外两个位置：宏在 `*` 前面、宏在 `*` 与名字之间。判据与读取器各补一半——
**读取器那一半正是 B98 撤回时缺的**（那时错误从 col 16 推到 col 51 却没修好，因为"宏在 `*` 前面"没人读）。
**量到**：带 seeds 455 干净 440 → **442** / 报错 15 → **13**，逐文件对照只有 `exception` 与 `combaseapi.h` 消失、无一反向。

**B100（初始化式两侧的接缝 + `__attribute` 少一个下划线的拼法）—— +2 个文件**：两件事，都不是"缺一条大规则"，
而是**探针把成因指错了地方**那一类。① `ext/concurrence.h:58` 与 `bits/stl_algobase.h:906` 都把 `#if` 写在 `=` 与
初始化式**之间**（一个变量的值写在三条分支里 / 一个 `\` 续行的 `__has_builtin` 条件），而 `finish_init_declarator`
的 `Assign` 分支直接叫 `parse_initializer_clause`：表达式规则没有 `#` 的读法，首错就打在**指令那一行**上
（907:0、59:0）——这也是它们被归成"指令位置"一族的原因。接缝本身是枚举量、模板形参表、形参表、花括号初始化式
**早就各自有**的那一条（B23），唯独声明自己的 `=` 漏了；做法是**值的前后各加一个指令环**，两个环都在 `Initializer`
节点里面（和"实参表里逗号两侧都要接缝"同理）。② `parallel/compatibility.h:49` 的 `__attribute`（**一个**尾下划线）
不在 `at_an_attribute` 的拼法表里，于是被读成宏形说明符、声明跟着丢；`g++ -std=c++17` 接受同一行，拼法属于编译器
而不属于文件，加进去即可。**量到**：455 那份带 seeds 干净 438 → **440** / 报错 17 → **15**，不带 seeds 干净
437 → **439** / 报错 18 → **16**（"不带 seeds"的基线是**临时把这两条规则关掉**重新量的，不是拿旧运行凑的），
**两种模式逐文件对照都只有 `concurrence.h` 与 `compatibility.h` 消失、无一反向**。`stl_algobase.h` 没修好，但首错
**往后走**了（907:0 → 912:5，两种模式一样）——接缝读进去以后撞上的是 B77 特意不读的"一个定义写在两个分支里"
（`#else` 的值是同一个声明符的第二个初始化式），这条不动。断言两条
（`gaps.rs::a_directive_may_stand_on_either_side_of_a_declarations_initializer`、
`the_attribute_spelling_with_one_trailing_underscore_is_an_attribute`），后者的反例是**形状**：没有括号跟着的
`__attribute` 仍然是名字（`int __attribute = 1;` 读成变量、零个 `AttributeList`）。

**B99（默认实参里的三目：type-id 只在"表还能继续"时才算答案）**：`bits/ranges_util.h:265-267` 的默认实参是
`sized_sentinel_for<_Sent, _It> ? subrange_kind::sized : subrange_kind::unsized`——**头是个类型的条件表达式**，
而 `parse_a_default_value` 先试 type-id、成功就返回，把 `?` 留给参数表 ⇒ ``expected `,` or `>` ``。改成：type-id
只有在"参数表还能继续"（`,`/`>`/`>>`/`#`）时才算答案，否则同一批 token 按表达式读。复现干净、`gaps.rs` 有正反两条断言。
**但语料读数一条没动**（455 仍 438/17）：那个文件的 267:6 是**带环境才出现**的首错（`cpp_dump` 直接读该文件时首错在
437 行），成因与复现不是同一处——这一条按"读数变了才算修好"的标准**不算修好**，只算了"读法正确且有断言"。
**同族的另外两条与它们的切入点**（都写在 `grammar-gaps.md` 的族清单里）：`tuple:2438` 是"括号 + 关系运算符 +
`sizeof`"；`type_traits:987` 是类头规则试过基类子句后失败、回退成变量声明，要等**实参表读取器加一句"为什么拒绝"的
打印**才能继续。`compatibility.h` 那一行用三轮探针缩到了"`extern "C"` + **属性宏**"（单独声明、属性关键字、属性宏
单独都干净，只有这个组合坏）。

**B98（指针限定符位置上的空体宏）—— +1 个文件**：`basetsd.h:16` 的 `#define POINTER_32` 在 64 位
目标上是**空体**，用法是 `(void *POINTER_32) p` ✗；而**文件自己的**空体 `#define` 一直没有被记录（B90 撤回过那半），
规则问不出"这个名字展开成什么"。三处：记进 `macro_bodies_empty`（与有形状的体分开存，空体顶不掉有形状的）、
`macro_body_kinds_at` 回答 `Some([])`（"什么都没有" ≠ "没人说过"）、消费者只有 `eat_cv_qualifiers` 一处且很窄
（`*` 后面的限定符位置）。**量到（带 seeds）**：455 那份 干净 437 → **438**、报错 18 → **17**，**只有 `basetsd.h` 变化、
无一反向**；不带 seeds 的 435/20/86 不动；B91 那条"空体在声明里不接受"的边界没动。**附带撤回**：`(STDAPICALLTYPE *NAME)`
（`combaseapi.h:358`）的判据与读取器写了、把错误从 col 16 推到 col 51，但**没修好**、语料一条没动 ⇒ 撤回；下一刀是
**读掉宏之后的名字登记**（既有的 `(MACRO NAME)` 那条路是干净的，差别就在这里）。**剩下 17 个按族分**：模板实参里的
`<` 当运算符（`type_traits`/`ranges_util.h`/`tuple`，最大一族）、指令位置（`stl_algobase.h`/`concurrence.h`/`omp.h`/
`lwpintrin.h`）、属性宏 + `extern "C"`（`compatibility.h`，复现已缩到一行）、以及九条单条。
