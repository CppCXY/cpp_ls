...
Rust 侧 `cargo test --workspace` = **1025 个测试 / 34 个套件全绿**。

**门禁三条 + 一条**（改完必须全绿，`index-design.md` §门禁有同样的表）：

```bash
cargo test --workspace                     # 1025 个测试，34 个套件
...
词法 vs 解析、以及重命名会改几处）、`examples/condition_reach.rs`（**条件求值**：闭包里每个条件 include 的形状、
...
**条件求值的第一半做完了**（第十五轮，见 §3.5c）：摘要存**问题**（分支链、正文、嵌套，`CODEC_VERSION` 11），
查询时用（编译器 `-dM` 预定义 + 编译数据库的 `-D`/`-U` + 本文件该点之上的无条件事实）求值成
`Active`/`Inactive`/`Unknown`，而"我不知道"（`Lookup::Unanswered`）与"没定义"从此是**两个答案**——`#if defined(X)`
和 `#if X` 不再把前者读成 `0`。量到的：459 文件的闭包里 **486 个条件 include 里 85 个被判定**（24 取到、61 跳过），
`-DX` 让引用从"可能"变"确定"，`#if 0` 后面的 include 不再跟进。方向鉴定仍在
[`parser-assessment.md`](parser-assessment.md) §7（"真正没买的只有一样——条件没有值"，这一轮把它买了一半）。

**接着做的顺序**：

1. **条件求值的第二半**（§3.5c）：把**闭包自己定义的宏**按翻译顺序喂进环境（97 个名字测试 + 头文件 feature 宏），
   前提是先把"输入完整吗"答清楚（未解析的 include / 没索引的文件 / 没人告诉的 `-D` 各是一格）。同时
   `__has_include`（47 处）只要让 `parse_condition` 认识它就能判定——那是个**文件问题**，解析器已经能答。
2. **parser 长尾**：120 个失败文件按首错归类（第 11 条），其中 B42/B43 已登记。它服务的仍然是"宏住的那批文件"
   （`winnt.h` 家族的四个宏卡在这一格）。
3. **其余语义边界**：类类型下标、模板实参成员、`auto` 返回、`using Base::f;`、运算符重载——`index-design.md`
   末尾那张表就是队列。
4. **宏展开（P4）**等某个问题需要它——第十五轮之后它的输入从"没有"变成了"有一个三值的分支判定"；
   **LSP 二进制**最后（客户端会通知我们，见 §3.6）。
