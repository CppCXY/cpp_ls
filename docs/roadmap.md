...
`干净 111 / 报错 17`，消息总数 **91**；每文件错误数 `干净 111 | 只有一个 5 | 两到五个 8 | 超过五个 4`；
`%TEMP%\cppls-indexed.txt` 的 455 个文件（分析闭包）——`干净 429 / 报错 26`，消息总数 **129**。
Rust 侧 `cargo test --workspace` = **1055 个测试 / 34 个套件全绿**。
...
一路下来干净 87 → **111** / 342 → **429**，消息 802 → **91** / 2359 → **129**。B66/B67/B69 是"再补一条接缝/一条
...
B76   **干净 +2 / +3，消息 −28 / −18**       头文件里的宏**自己就是一条语句**（没有分号）；判据次序错一次，
                                              把 B73 的形状抢走，被已有测试抓住std_query（语义查询端到端）9/9 —— 八条都没动它
```
